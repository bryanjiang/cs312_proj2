...
prison (Move move (mine, others))
    | length mine + length others == 10 && (win mine others) < 0  = EndOfGame 1      -- i lose
    | length mine + length others == 10 && (win mine others) > 0  = EndOfGame 2     -- i win
...
simple_player _ (ContinueGame _ avail) = head [e | e <- [0,0,0,0,0], e `elem` avail]
